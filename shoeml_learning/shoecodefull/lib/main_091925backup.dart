...
class _ShoeMLAppState extends State<ShoeMLApp> {
  ...
  void _onBleDisconnected([Object? error]) {
    if (!mounted) return;
    final msg = (error != null) ? 'Device disconnected: $error' : 'Device disconnected';
...
_ble.dispose();
  ...
  onPressed: () async {
    try {
      ...
      await intent.launch();  // opens system Bluetooth settings (Android)
    } catch (_) {
...
class ShoeBle {
  ...
  void _pushTemp(double v) { if (!_tempCtrl.isClosed) _tempCtrl.add(v); }
...
disconnect();
  ...
  Future<void> _subscribeAll() async {
    ...
    // === injected: subscribe temperature characteristic ===
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _bind() {
    _s1 = widget.ble.connection$.listen((c) {
      if (!mounted) return;
      setState(() { conn = c; });
    }, onError: (e, st) => _onBleDisconnected(e), onDone: () => _onBleDisconnected(null), cancelOnError: true);
    ...
    _s6 = widget.ble.steps$.stream.listen((v) {
      if (!mounted) return;
      setState(() { steps = v; });
    }, onError: (e, st) => _onBleDisconnected(e), onDone: () => _onBleDisconnected(null), cancelOnError: true);
    _s7 = widget.ble.cadence$.stream.listen((v) {
      if (!mounted) return;
      setState(() { cadence = v; });
    }, onError: (e, st) => _onBleDisconnected(e), onDone: () => _onBleDisconnected(null), cancelOnError: true);
    _s8 = widget.ble.stride$.stream.listen((v) {
      if (!mounted) return;
      setState(() { stride = v; });
    }, onError: (e, st) => _onBleDisconnected(e), onDone: () => _onBleDisconnected(null), cancelOnError: true);
