...
class ShoeBle {
  ...
  // ---------------- Core ----------------
  // FIX 1: constructor has no arguments in 5.x
  ...
  StreamSubscription<List<int>>?
      _sRoll, _sPitch, _sYaw, _sTemp, _sSteps, _sCad, _sStride, _sDh;
  ...
  // Expose BLE status & a readiness wait (Android 12+ quirk)
  ...
  /// Scan that prefers the last saved id/name; otherwise filters by service.
  ...
  /// Robust connect with re-entrancy guards. Never hard-disconnect first.
  ...
  /// Start notifications after a safe connect (subscribe: false).
  ...
  /// Try to reconnect to last device; if unknown, scan and connect.
  Future<void> tryReconnectLast({Duration delay = const Duration(milliseconds: 300)}) async {
    ...
    final found = await scanOncePreferLast();
    if (found != null) {
  ...
  Future<void> _subscribeAll(int myEpoch) async {
    ...
    // FIX 2: use the safe top-level helper you already have
    _sTemp   = await sub(tempUuid,   (b) { final v = decodeTempOrNull(b); if (v != null) _tempCtrl.add(v); });
  ...
  void startAutoScanner({
    ...
    // immediate attempt
    () async => await tryReconnectLast(delay: firstDelay)();
  ...
  Future<void> _saveLast(String id, {String? name}) async {
    try {
      ...
      debugPrint("[ShoeBle] saved last id=$id name=${name ?? ''}");
    } catch (e) {
      debugPrint("[ShoeBle] saveLast error: $e");
    }
  }
  ...
  Future<void> _loadLast() async {
    try {
      ...
      if (await f.exists()) {
        ...
        debugPrint("[ShoeBle] loaded last id=${_lastId ?? ''} name=${_lastName ?? ''}");
      }
    } catch (e) {
      debugPrint("[ShoeBle] loadLast error: $e");
    }
  }
...
class _GraphsPageState extends State<GraphsPage> {
  ...
  void initState() {
   ...
   _sTemp = widget.ble.temp$.listen((v) {
    if (v == null) return;
    final t = DateTime.now().difference(_t0!).inMilliseconds / 1000.0;
    _push(_temp, t, v);            // reuse your existing _push helper
    if (mounted) setState(() {});
   });
  }
...
SizedBox(
  ...
  return SizedBox(
    ...
    child: FlutterMap(
      mapController: mc,
      options: MapOptions(
        // Any valid initial target; we'll immediately fit to bounds.
        initialCenter: track.isNotEmpty ? track.first : const LatLng(20, 0),
        initialZoom: 14,
        interactionOptions: const InteractionOptions(
          // disable rotate for easier viewing in a sheet
          flags: InteractiveFlag.all & ~InteractiveFlag.rotate,
        ),
        onMapReady: () {
          if (track.length >= 2) {
            // Fit camera to the whole route once the map is ready.
