...
void dispose() {
  ...
  // 1) If we are about to START collecting, require BLE connection
  ...
  if (_collecting) {
    ...
    // If no samples were ever received, do NOT save anything
    if (_buffer.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No sensor samples received – nothing to save.'),
        ),
      );
      return;
    }
                ...
                children: [
                  ...
                  FilledButton.icon(
                    onPressed: _collecting
                       ...
                       : (canStart ? _toggleCollect : null), // disable Start if not connected
