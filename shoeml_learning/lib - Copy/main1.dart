...
void _onNewLiveRoll(double v) {
  ...
  void _tryClassifyLive() {
    ...
    // ---- Ensemble: average probability per label across models ----
    final Map<String, double> sumProbs = {};
    final Map<String, int> counts = {};

    for (final entry in _liveModels) {
      final probs = entry.model.predictProbs(feat); // label -> p
      probs.forEach((lbl, p) {
        sumProbs[lbl] = (sumProbs[lbl] ?? 0.0) + p;
        counts[lbl] = (counts[lbl] ?? 0) + 1;
      });
    }

    if (sumProbs.isEmpty) return;

    String? bestLabel;
    double bestScore = -1.0;

    sumProbs.forEach((lbl, s) {
      final c = counts[lbl] ?? 1;
      final mean = s / c; // mean probability across models
      if (mean > bestScore) {
        bestScore = mean;
        bestLabel = lbl;
      }
    ...
    if (!mounted) return;
    setState(() {
      _liveActivity = bestLabel;
      _liveConf = bestScore > 0 ? bestScore : null;
    });
  }
