...
// ---------- BLE callbacks (safer reconnect handling) ----------
static volatile bool bleClientConnected = false;
static volatile bool bleRestartAdvPending = false;
static uint32_t bleRestartAdvAtMs = 0;

static void handleBleConnected() {
  bleClientConnected = true;
  bleRestartAdvPending = false;
  LOGLN("[BLE] Connected");
}

static void handleBleDisconnected(int reason) {
  bleClientConnected = false;
  bleRestartAdvPending = true;
  bleRestartAdvAtMs = millis() + 200; // schedule advertising restart in loop()
  LOGF("[BLE] Disconnected reason=%d -> schedule re-adv\n", reason);
}
...
void setupBLE(){
  ...
  if (adv) {
    NimBLEAdvertisementData a;
    a.setFlags(0x06);
    a.addServiceUUID(BLE_SERVICE_UUID);
    adv->setAdvertisementData(a);

    NimBLEAdvertisementData s;
    s.setName("MADEPLUS SMART SHOE");
    adv->setScanResponseData(s);

    adv->start();
  } else {
    LOGLN("[BLE] ERROR: advertising object is null");
  }
}
...
// ---------- Loop ----------
void loop(){
  // Blink + safe re-adv from loop context only
  static uint32_t lastBlink=0;
  if(millis()-lastBlink>500){ lastBlink=millis(); digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN)); }

  const unsigned long now = millis();

  if (!bleClientConnected && bleRestartAdvPending &&
      (int32_t)(now - bleRestartAdvAtMs) >= 0) {
    bleRestartAdvPending = false;
    NimBLEAdvertising* adv = NimBLEDevice::getAdvertising();
    if (adv) {
      adv->start();
      ...
      LOGLN("[BLE] ERROR: advertising object null during restart");
    }
  }
  ...
  if(now - lastNotify >= notifyPeriodMs){
    ...
    // BLE notify only while a client is connected
