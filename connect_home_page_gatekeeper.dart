...
class _ConnectHomePageState extends State<ConnectHomePage> {
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      // Later launches:
      // stay on home page and try reconnect in the background.
      if (seen) {
  ...
  Future<void> _startReconnectInBackground() async {
    ...
    try {
      if (widget.reconnectLastIfAny != null) {
        ...
        // Do not navigate here unless the connection stream already said connected.
        // The connection listener is the source of truth.
      } else {
        if (!mounted) return;
        setState(() {
          _isTryingReconnect = false;
  ...
  Future<void> _ensurePerms() async {
    ...
    try {
      ...
      // Keep UI alive even if some permissions are ignored.
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final laterLaunchLocked = _hasSeenWelcome && !_isConnected;
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: laterLaunchLocked
                    ? null
                    : () => _goToDashboard(markSeen: true),
            ...
            Text(
              _hasSeenWelcome
                  ? 'When the sensor is disconnected, the app stays on this home page. As soon as it reconnects, the dashboard opens automatically.'
                  : 'First launch stays on this welcome page. After that, the app will wait here until the sensor reconnects, then open the dashboard automatically.',
