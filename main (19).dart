...
class ShoeBle {
  ...
  final ValueNotifier<int> sessionStepsNotifier = ValueNotifier<int>(0);
 ...
 Future<bool> reconnectLastIfAny() async {
  ...
  Future<void> _subscribeAll() async {
    ...
    _sSteps =await sub(stepsUuid, (b){
      ...
      if(_cumSteps < 0){
        ...
        sessionStepsNotifier.value = _sessionSteps;
        _onFirmwareStep();
        _cumSteps = cnt;
      }
    });
  ...
  void _resetAnalytics(){
    ...
    _cumSteps=-1; _baseSteps=0; _sessionSteps=0; _cumPitchSteps=0;
    sessionStepsNotifier.value=0; _lastStepWall=-1; _lastCadence=0; _lastEmittedCadence=0; _stepWallTimes.clear();
    // Legacy fields
    _tA=0;_baseline=0;_lastCentered=0;_lastStepT=-1;_lastStride=0;
    _baselineInit=false; _lastPitchTs=null;
    // _stepsACtrl retired — dashboard uses sessionStepsNotifier
  ...
  void _pitchAnalytic(double deg){
    ...
    // α=0.05 → 2s time constant at 10 Hz. Old 0.01 was 10s — missed early steps.
    else{_baseline+=0.05*(deg-_baseline);}
    final c=deg-_baseline;
    // 3° threshold (was 5°) catches smaller steps and slow walking.
    if(_lastCentered<=0&&c>0&&c.abs()>=3&&(_lastStepT<0?999:_tA-_lastStepT)>=0.3){
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _bind(){
    ...
    widget.ble.sessionStepsNotifier.addListener(_onStepsChanged);
  ...
  void _onStepsChanged(){
    if(mounted) setState(()=>steps=widget.ble.sessionStepsNotifier.value);
  }
  void dispose(){
    widget.ble.sessionStepsNotifier.removeListener(_onStepsChanged);
