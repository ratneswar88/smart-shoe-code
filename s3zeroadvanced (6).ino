...
#define BLE_CHAR_PHASE_UUID     "0000c005-0000-1000-8000-00805f9b34fb"
...
void setupBLE(){
  ...
  uint8_t ph0=0; chPhase->setValue(&ph0,1);
...
void loop(){
  ...
  if(now - lastSample >= samplePeriodMs){
    ...
    if(state != newState){
      uint8_t newPh=(newState==SWING)?1:0; chPhase->setValue(&newPh,1); chPhase->notify();
  ...
  if(now - lastNotify >= notifyPeriodMs){
    ...
    uint8_t phSnap=(state==SWING)?1:0; chPhase->setValue(&phSnap,1); chPhase->notify();
