...
#include <Arduino.h>
#include <esp_task_wdt.h>  // Watchdog — prevents hang on I2C lockup
#include <math.h>
#include <Wire.h>
...
#define BLE_CHAR_PHASE_UUID     "0000c005-0000-1000-8000-00805f9b34fb" // gait phase: 0=STANCE 1=SWING
...
// ---------- BLE globals ----------
NimBLECharacteristic *chRoll=nullptr,*chPitch=nullptr,*chYaw=nullptr,*chTemp=nullptr,*chSteps=nullptr,*chCad=nullptr,*chStrideL=nullptr,*chAltDH=nullptr,*chPhase=nullptr,*chCmd=nullptr,*chAscii=nullptr;
...
// Min physiological step interval — humans cannot step faster than ~240 spm.
// Guards against phantom counts from drops, vibration, or false ZUPT triggers.
static const unsigned long MIN_STEP_MS = 250UL;
...
// ---------- Non-blocking baro state machine ----------
// msReadPT() blocks for ~20 ms (two 10 ms ADC conversions).  At 10 Hz notify
// rate that steals 200 ms/s from the 200 Hz sample loop.
// Instead, we run a state machine in baroTick() with no delays.
enum BaroSt { BARO_IDLE=0, BARO_WAIT_D2, BARO_WAIT_D1 };
static BaroSt baroSt = BARO_IDLE;
static uint32_t baroT0 = 0;
static uint32_t baroD2raw = 0;
static const uint32_t BARO_CONV_MS      = 11; // MS5611 OSR=4096: 9.04 ms + margin
static const uint32_t BARO_INTERVAL_MS  = 100; // ~10 Hz refresh
static uint32_t lastBaroUpdateMs = 0;
...
void baroTick() {
  ...
  switch (baroSt) {
    case BARO_IDLE:
      if (now - lastBaroUpdateMs >= BARO_INTERVAL_MS) {
        if (msWrite(MS5611_CMD_CONV_D2_4096)) {
          baroT0 = now;
          baroSt = BARO_WAIT_D2;
        }
      }
      break;
    case BARO_WAIT_D2:
      if (now - baroT0 >= BARO_CONV_MS) {
        if (!msReadADC(baroD2raw)) { baroSt = BARO_IDLE; break; }
        if (msWrite(MS5611_CMD_CONV_D1_4096)) { baroT0 = now; baroSt = BARO_WAIT_D1; }
        else baroSt = BARO_IDLE;
      }
      break;
    case BARO_WAIT_D1: {
      if (now - baroT0 >= BARO_CONV_MS) {
        uint32_t D1raw = 0;
        if (!msReadADC(D1raw)) { baroSt = BARO_IDLE; break; }
        int32_t dT   = (int32_t)baroD2raw - ((int32_t)C_ms[5] << 8);
        int64_t OFF  = ((int64_t)C_ms[2] << 16) + (((int64_t)C_ms[4] * dT) >> 7);
        int64_t SENS = ((int64_t)C_ms[1] << 15) + (((int64_t)C_ms[3] * dT) >> 8);
        int32_t TEMP = 2000 + (int32_t)(((int64_t)dT * (int32_t)C_ms[6]) / 8388608);
        int32_t P    = (int32_t)(((((int64_t)D1raw * SENS) >> 21) - OFF) >> 15);
        lastPress  = P / 100.0f;
        lastTempC  = TEMP / 100.0f;
        float alt  = pressureToAltitude(lastPress);
        // Update per-step Δh only when a step just happened
        // (heel-strike handler sets prevStepAlt_m; done without blocking)
        lastAlt_m  = alt;
...
void setupBLE(){
  ...
  chPhase   = svc->createCharacteristic(BLE_CHAR_PHASE_UUID,   NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::NOTIFY);
  ...
  uint8_t ph0 = 0; chPhase->setValue(&ph0, 1); // 0 = STANCE at boot
...
void setup(){
  ...
  // Watchdog: extend timeout to 8 s to cover worst-case I2C recovery.
  // Use esp_task_wdt_reconfigure() — safe on Arduino core 3.x (IDF v5)
  // because the core already initializes the WDT and subscribes the main
  // task. Calling deinit/init/add would cause a double-subscribe panic
  // that corrupts the USB stack and bricks the device until BOOT+RESET.
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  {
    ...
    esp_task_wdt_reconfigure(&wdt_cfg); // change timeout, keep existing subscriptions
  }
#endif
  // Note: esp_task_wdt_reset() in loop() feeds the WDT the core subscribed.
}

// ---------- Loop ----------
void loop(){
  esp_task_wdt_reset();  // feed the watchdog every iteration
  baroTick();            // advance non-blocking baro state machine
  ...
  if(now - lastSample >= samplePeriodMs){
    ...
    // Stance logic — threshold scales with cadence so running stance
    // (higher foot angular velocity at contact) isn't mis-classified as swing.
    // At 0 spm: threshold = TH_GYRO_DPS. At 200 spm: +15 dps. Max 55 dps.
    float adaptiveGyroTh = fminf(TH_GYRO_DPS + (cadence_spm / 200.0f) * 15.0f, 55.0f);
    bool zuptCandidate = (gyroMag_dps < adaptiveGyroTh) && (varA_mps2_2 < TH_VAR_A);
    ...
    if(state != newState){
      // Notify phase immediately on transition for minimum latency.
      uint8_t newPh = (newState == SWING) ? 1 : 0;
      chPhase->setValue(&newPh, 1);
      chPhase->notify();

      if(state==SWING && newState==STANCE){        // Heel-strike
        unsigned long tnow = now;
        // Min step interval guard — skip false detections.
        bool validStep = (lastHS_ms == 0 || (tnow - lastHS_ms) >= MIN_STEP_MS);
        if(!validStep){
          // False detection: reset ZUPT state but don't count or update cadence.
        ...
        // Baro Δh per step — use the most recent non-blocking measurement
        // (baroTick() keeps lastAlt_m fresh at ~10 Hz; no delay() needed)
        if (msPresent && !isnan(lastAlt_m)) {
          float alt = lastAlt_m;
          if (!isnan(prevStepAlt_m)) {
            lastStepAltDelta_m = alt - prevStepAlt_m;
          } else {
            lastStepAltDelta_m = 0.0f;
          }
          prevStepAlt_m = alt;
        } else {
          // Temp fallback from MPU die temperature when baro absent
    ...
    // Model-based stride estimate — fallback when ZUPT integration diverges
    // (accumulated horizontal displacement > 2.5 m is physically implausible).
    float dv = swingVertPeak - G_MPS2;
    lastModelLen_m = (dv > 0.0f) ? (kGain * sqrtf(max(0.0f, dv))) : 0.0f;
    if(lastStrideLen_m > 2.5f || (lastStrideLen_m <= 0.0f && lastModelLen_m > 0.0f)){
      lastStrideLen_m = lastModelLen_m;
    }
  }
  ...
  if(now - lastNotify >= notifyPeriodMs){
    ...
    // Baro values are already kept fresh by baroTick() — no blocking reads here.
    // Cadence decay to zero after 2 s of no steps.
    if(lastHS_ms > 0 && (now - lastHS_ms) > 2000UL){ cadence_spm = 0.0f; }
    // ASCII & floats out
    ...
    // Phase also re-notified at 10 Hz so late-connecting clients get it.
    uint8_t phSnap = (state == SWING) ? 1 : 0;
    chPhase->setValue(&phSnap, 1); chPhase->notify();
