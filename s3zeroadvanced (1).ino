...
void setup(){
  ...
  // Watchdog: 8 s timeout (generous to survive I2C bus lockup in loop()).
  // esp_task_wdt_deinit() first — some Arduino cores pre-subscribe the
  // main task; calling init+add without deinit causes a double-subscribe
  // panic that silently prevents BLE from ever advertising.
  esp_task_wdt_deinit(); // safe even if WDT was not previously inited
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  {
    const esp_task_wdt_config_t wdt_cfg = {
      .timeout_ms    = 8000,  // 8 s — covers worst-case I2C timeout
      .idle_core_mask = 0,
      .trigger_panic  = true,
    };
