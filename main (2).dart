...
class ShoeBle {
  ...
  Future<void> reconnectLastIfAny() async {
    try{
      final id=(await SharedPreferences.getInstance()).getString(_prefsKey);
      if(id==null) return;
      _shouldReconnect=true; _deviceId=id; _resetAnalytics();
      await _attempt(id);
    } catch(_){}
  }
  ...
  void dispose(){
    _shouldReconnect=false;_reconnectTimer?.cancel();_stopRssi();
    try{ _connSub?.cancel(); } catch(_){}
    _connSub=null;
    _sRoll=_sPitch=_sYaw=_sTemp=_sSteps=_sCad=_sStride=_sDh=null;
    for(final c in [_connState,_rssiCtrl,_roll$,_pitch$,_yaw$,_steps$,_cadence$,_stride$,_dh$,
                    _tempCtrl,_stepsACtrl,_cadenceACtrl,_strideACtrl]){
      try{ c.close(); } catch(_){}
    }
  }
}
...
class _TrainingPageState extends State<TrainingPage> {
  ...
  @override void initState(){
    super.initState();
    _connSub=widget.ble.connection$.listen((s){ if(mounted) setState(()=>_conn=s); });
    _bind(); _loadModels();
  }
