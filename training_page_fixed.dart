...
// raw samples buffer for CSV/windows
  ...
  Widget _buildRouteMap() {
    // If we have no GPS points yet, show a friendly placeholder instead of a map
    if (_track.isEmpty) {
      return Container(
...
/// Return windows of samples, not a flattened list.
  ...
  Future<bool> _ensureLocationPermission() async {
    // Ensure that the device location service is enabled
    final serviceEnabled = await Geolocator.isLocationServiceEnabled();
    if (!serviceEnabled) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please enable location (GPS) in system settings')),
        );
      }
      return false;
    }
  ...
  Future<void> _startLocation() async {
    ...
    try {
      ...
      // Guard against NaN/Infinity values from the platform
      ...
      // ignore for now; user will just see no route
    }
