...
class _TrainingPageState extends State<TrainingPage> {
  ...
  Future<void> _trainAndSave() async {
    ...
    final stamp = DateFormat("yyMMdd_HHmmss").format(DateTime.now());
    final baseName = _modelNameCtrl.text.trim().isEmpty ? _label : _modelNameCtrl.text.trim();
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text("Trained acc ${(acc * 100).toStringAsFixed(1)}% • Saved $name")));
  }
        ...
        children: [
          Card(
            child: ListTile(
              ...
              subtitle: const Text("Pick a label, Start/Stop capture, CSV auto-saves on stop."),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Text("Train & Save Model", style: TextStyle(fontWeight: FontWeight.bold)),
                  ...
                  const SizedBox(height: 8),
                  const Text("Windows: 3s / hop 1.5s • Features: mean/var/RMS + cadence/stride/temp"),
                  const SizedBox(height: 8),
                  FilledButton.icon(
          ...
          Card(
            child: ListTile(
              ...
              subtitle: const Text("Open Files → Android/data/<pkg>/files/shoeml/sessions"),
              trailing: IconButton(
          ...
          // DISABLE MAP DURING COLLECTION - Only show placeholder
          Card(
            child: Padding(
              padding: const EdgeInsets.all(12),
              child: Column(
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Column(
                          ...
                          children: [
                            Icon(
                              _collecting ? Icons.location_on : Icons.map_outlined,
                              size: 48,
                              color: _collecting ? Colors.green : Colors.grey[400],
                            ),
                            const SizedBox(height: 8),
                            Text(
                              _collecting 
                                  ? 'GPS tracking: ${_track.length} points'
                                  : 'Start collecting to track route',
                              style: TextStyle(
                                color: _collecting ? Colors.green[700] : Colors.grey[600],
                                fontWeight: _collecting ? FontWeight.w600 : FontWeight.normal,
                              ),
                            ),
                            if (_track.isNotEmpty && !_collecting)
                              Padding(
                                padding: const EdgeInsets.only(top: 4),
                                child: Text(
                                  'Last session: ${_track.length} GPS points',
  ...
  List<List<_RawSample>> _windows(List<_RawSample> buf, {double winSec = 3.0, double hopSec = 1.5}) {
  ...
  List<double> _features(List<_RawSample> w) {
    ...
    List<double> stats(List<double> x) {
      ...
      final varr = x.map((v) => (v - mean) * (v - mean)).reduce((a, b) => a + b) / n;
  ...
  Future<void> _startLocation() async {
    ...
    try {
      ...
      if (!pos.latitude.isNaN && !pos.longitude.isNaN &&
          !pos.latitude.isInfinite && !pos.longitude.isInfinite &&
          pos.latitude.abs() <= 90 && pos.longitude.abs() <= 180) {
        setState(() => _track = [LatLng(pos.latitude, pos.longitude)]);
      }
    } catch (_) {}
  ...
  Future<File?> _saveGeoJson(File csvFile) async {
    try {
      ...
      if (validTrack.isEmpty) return null;
      final dir = csvFile.parent;
      final name = csvFile.uri.pathSegments.last.replaceAll('.csv', '.geojson');
      final coords = validTrack.map((p) => [p.longitude, p.latitude]).toList();
      final fc = {
        "type": "FeatureCollection",
        "features": [
          {
            "type": "Feature",
            "properties": {"label": _label, "source": "ShoeML"},
  ...
  Future<File?> _captureMapPng(File csvFile) async {
    // Skip map PNG capture since we're not showing the map
    return null;
  }
}
...
class SoftmaxClassifier {
  ...
  int predict(List<double> x) {
    final z = List<double>.generate(
        C, (k) => List<double>.generate(D, (j) => W[k][j] * x[j]).reduce((a, b) => a + b) + b[k]);
    final p = _softmax(z);
    int arg = 0;
    double best = p[0];
  ...
  void fit(List<List<double>> X, List<int> y,
      {int epochs = 50, double lr = 0.05, double l2 = 1e-4, int batch = 64}) {
    final n = X.length;
