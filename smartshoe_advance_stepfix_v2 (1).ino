...
unsigned long lastHS_ms = 0; // last detected step time
float cadence_spm = 0.0f;    // steps per minute
unsigned long hsTimes[8]; int hsIdx=0;

// Faster step detector for UI responsiveness:
// count steps from pitch motion immediately, keep ZUPT for stride/altitude.
float stepPitchBaseline = 0.0f;
bool  stepPitchBaselineInit = false;
float stepLastCentered = 0.0f;
bool  stepArmed = false;
float STEP_PITCH_THRESH_DEG = 4.0f;      // 3.5–5.0 works well for walking
float STEP_ARM_THRESH_DEG   = 2.0f;      // hysteresis arm level
float STEP_GYRO_MIN_DPS     = 12.0f;     // ignore tiny sway / stillness
uint32_t STEP_DEBOUNCE_MS   = 320;       // max ~187 spm
...
// msReadPT() blocks ~20 ms. Instead, advance a state machine each loop().
...
static const uint32_t BARO_CONV_MS     = 11;
static const uint32_t BARO_INTERVAL_MS = 100;
static uint32_t lastBaroUpdateMs = 0;

void baroTick(uint32_t now) {
  if (!msPresent) return;
  switch (baroSt) {
    case BARO_IDLE:
      if (now - lastBaroUpdateMs >= BARO_INTERVAL_MS) {
        if (msWrite(MS5611_CMD_CONV_D2_4096)) { baroT0 = now; baroSt = BARO_WAIT_D2; }
      }
      break;
    case BARO_WAIT_D2:
    ...
    case BARO_WAIT_D1: {
      if (now - baroT0 >= BARO_CONV_MS) {
        ...
        lastAlt_m  = pressureToAltitude(lastPress);
...
public:
  void onWrite(NimBLECharacteristic* c) {
    ...
    if(v.startsWith("step_th=")){ STEP_PITCH_THRESH_DEG = v.substring(8).toFloat(); c->setValue((uint8_t*)"OK",2); return; }
    if(v.startsWith("step_arm=")){ STEP_ARM_THRESH_DEG = v.substring(9).toFloat(); c->setValue((uint8_t*)"OK",2); return; }
    if(v.startsWith("step_db=")){ STEP_DEBOUNCE_MS = (uint32_t)constrain(v.substring(8).toInt(), 200, 800); c->setValue((uint8_t*)"OK",2); return; }
  }
};
...
static void registerStepEvent(unsigned long tnow) {
  stepCount++;

  if(lastHS_ms > 0) {
    unsigned long dt_ms = tnow - lastHS_ms;
    if(dt_ms > 0) {
      hsTimes[hsIdx & 7] = tnow;
      hsIdx++;
      ...
      if(dSum > 0) {
        float rawCad = 60000.0f * (float)(k - 1) / (float)dSum;
        // EMA α=0.25 — smooth cadence but keep it responsive.
        cadence_spm = (cadence_spm < 1.0f)
            ? rawCad
            : cadence_spm + 0.25f * (rawCad - cadence_spm);
      }
    }
  }
  lastHS_ms = tnow;
...
void setup(){
  ...
  // Erase NVS unconditionally — clears stale BLE bonding data that
  // causes NimBLEDevice::init() to crash on this board/IDF combination.
  nvs_flash_erase();
  nvs_flash_init();
...
// ---------- Loop ----------
void loop(){
  baroTick(millis());  // advance non-blocking baro state machine
  // Blink + re-adv
  ...
  if(now - lastSample >= samplePeriodMs){
    ...
    // Faster dedicated step detector:
    // count from pitch motion immediately instead of waiting for strict
    // SWING->STANCE confirmation. This drives step count and cadence only.
    if(!stepPitchBaselineInit){
      stepPitchBaseline = pitch_deg;
      stepPitchBaselineInit = true;
      stepLastCentered = 0.0f;
      stepArmed = false;
    } else {
      // α=0.05 → 0.1s time constant at 200 Hz (was 0.01 → 0.5s, missed early steps)
      stepPitchBaseline += 0.05f * (pitch_deg - stepPitchBaseline);
      float stepCentered = pitch_deg - stepPitchBaseline;

      // Arm when the pitch excursion goes negative enough, then fire when it
      // swings positive past the main threshold. This hysteresis avoids the
      // impossible "zero-cross and large amplitude at the same instant" bug.
      if(stepCentered <= -STEP_ARM_THRESH_DEG){
        stepArmed = true;
      }

      bool crossedHigh = (stepLastCentered < STEP_PITCH_THRESH_DEG) &&
                         (stepCentered >= STEP_PITCH_THRESH_DEG);
      bool debounceOk  = (lastHS_ms == 0) || ((now - lastHS_ms) >= STEP_DEBOUNCE_MS);
      bool motionGate  = (gyroMag_dps >= STEP_GYRO_MIN_DPS) || !zuptCandidate;

      if(stepArmed && crossedHigh && debounceOk && motionGate){
        registerStepEvent(now);
        stepArmed = false;
      }

      // Re-arm if the waveform falls back near baseline.
      if(stepCentered > -0.5f && stepCentered < 0.5f && (now - lastHS_ms) > 120){
        // no-op: keeps state stable around zero
      }

      stepLastCentered = stepCentered;
    }

    // Events + swing integration
    if(state != newState){
      if(state==SWING && newState==STANCE){
        // ZUPT heel-strike confirmation:
        // keep stride length / altitude updates here, but step count was already
        // updated by the faster dedicated detector above.

        // End of swing → stride length
        float stepHoriz_m = sqrtf(px*px + py*py); // ignore vertical component
        lastStrideLen_m = stepHoriz_m;

        // Baro Δh per step — use non-blocking baroTick() value
        if (msPresent && !isnan(lastAlt_m)) {
          if (!isnan(prevStepAlt_m)) { lastStepAltDelta_m = lastAlt_m - prevStepAlt_m; }
          else { lastStepAltDelta_m = 0.0f; }
          prevStepAlt_m = lastAlt_m;
        } else {
  ...
  if(now - lastNotify >= notifyPeriodMs){
    ...
    // Baro values kept fresh by baroTick() — no blocking reads here
