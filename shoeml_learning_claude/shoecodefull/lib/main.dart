...
/// ===== Safe listen helper (prevents setState after dispose) =====
StreamSubscription<T> safeListen<T>(
  State state,
  Stream<T> stream,
  void Function(T v) onData, {
  void Function(Object? error)? onDisconnect,
  bool setStateWrap = true, // false = no rebuild, just update data
}) {
...
class _ShoeMLAppState extends State<ShoeMLApp> {
  ...
  void _onBleDisconnected([Object? error]) {
    if (!mounted) return;
    final msg =
        (error != null) ? 'Device disconnected: $error' : 'Device disconnected';
          ...
          IconButton(
            ...
            onPressed: () async {
              try {
                ...
                await intent.launch(); // Android system Bluetooth settings
              } catch (_) {
                await Geolocator.openAppSettings(); // fallback / iOS
              }
            },
...
class ShoeBle {
  // PATCH: temperature stream
  final _tempCtrl = StreamController<double?>.broadcast();
  Stream<double?> get temp$ => _tempCtrl.stream;

  // === ADDED: resubscribe registry ===
  final Map<Uuid, void Function(List<int>)> _subHandlers = {};
  final Map<Uuid, StreamSubscription<List<int>>> _notifSubs = {};

  // === injected: temperature streaming ===
  void _pushTemp(double v) {
    if (!_tempCtrl.isClosed) _tempCtrl.add(v);
  }
  ...
  Future<void> _subscribeAll() async {
    ...
    await sub(tempUuid, (bytes) {
      try {
        final t = _decodeTemp(bytes);
        if (t != null) _pushTemp(t);
      } catch (_) {}
    });
    _sTemp =
        await sub(tempUuid, (b) => _tempCtrl.add(_f32Nullable(b)));
  ...
  void _startRssi() {
    ...
    _rssiTimer = Timer.periodic(const Duration(seconds: 2), (_) async {
      try {
        final r = await _ble.readRssi(_deviceId!);
        _rssi.add(r);
      } catch (_) {}
    });
  }
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _bind() {
    _s1 = safeListen<DeviceConnectionState>(
      this,
      widget.ble.connection$,
      (c) => conn = c,
      onDisconnect: _onBleDisconnected,
    );

    _s2 = safeListen<double>(
      this,
      widget.ble.roll$.stream,
      (v) => roll = v,
      onDisconnect: _onBleDisconnected,
    );
    _s3 = safeListen<double>(
      this,
      widget.ble.pitch$.stream,
      (v) => pitch = v,
      onDisconnect: _onBleDisconnected,
    );
    _s4 = safeListen<double>(
      this,
      widget.ble.yaw$.stream,
      (v) => yaw = v,
      onDisconnect: _onBleDisconnected,
    );
    _s5 = safeListen<double?>(
      this,
      widget.ble.temp$,
      (v) => tempC = v,
      onDisconnect: _onBleDisconnected,
    ...
    _s6 = safeListen<int>(
      this,
      widget.ble.steps$.stream,
      (v) => steps = v,
      onDisconnect: _onBleDisconnected,
    );
    _s7 = safeListen<double>(
      this,
      widget.ble.cadence$.stream,
      (v) => cadence = v,
      onDisconnect: _onBleDisconnected,
    );
    _s8 = safeListen<double>(
      this,
      widget.ble.stride$.stream,
      (v) => stride = v,
      onDisconnect: _onBleDisconnected,
    );
    _s9 = safeListen<double>(
      this,
      widget.ble.dh$.stream,
      (v) => dh = v,
      onDisconnect: _onBleDisconnected,
    );
    _s10 = safeListen<int>(
      ...
      child: ListView(
        children: [
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              _metric(cs, "Roll (°)", roll.toStringAsFixed(1),
                  Icons.rotate_90_degrees_ccw),
              _metric(cs, "Pitch (°)", pitch.toStringAsFixed(1),
              ...
              _metric(cs, "Cadence", "${cadence.toStringAsFixed(1)} spm",
                  Icons.speed),
              _metric(cs, "Stride", "${stride.toStringAsFixed(2)} m",
                  Icons.straighten),
              _metric(cs, "Elevation Δh", "${dh.toStringAsFixed(2)} m",
                  Icons.landscape),
            ],
          ),
...
class _TrainingPageState extends State<TrainingPage> {
  ...
  void _bind() {
    ...
    // These listeners don't need rebuilds; avoid setState for perf.
    _sr = safeListen<double>(
      this,
      widget.ble.roll$.stream,
      (v) {
        t += 0.1;
        _buffer.add(
            _RawSample(t, v, null, null, null, null, null, null));
      },
      setStateWrap: false,
    );
  ...
  Future<void> _trainAndSave() async {
    ...
    final labels = List<int>.filled(feats.length, _labels.indexOf(_label));
                      ...
                      child: Stack(
                        children: [
                          FlutterMap(
                            ...
                            options: MapOptions(
                              initialZoom: 16,
                              initialCenter: _track.isNotEmpty
                                  ? _track.last
                                  : const LatLng(0, 0),
                            ),
                            children: [
  ...
  Future<File?> _saveGeoJson(File csvFile) async {
    try {
      ...
      final temps =
          _buffer.map((s) => s.tempC).whereType<double>().toList();
      final tempAvg =
  ...
  Future<File?> _saveGpx(File csvFile) async {
    try {
      ...
      b.writeln(
          '<gpx version="1.1" creator="ShoeML" xmlns="http://www.topografix.com/GPX/1/1">');
      b.writeln('  <trk>');
      ...
      for (int i = 0; i < _track.length; i++) {
        ...
        b.writeln(
            '      <trkpt lat="${p.latitude}" lon="${p.longitude}"><time>${t}Z</time></trkpt>');
      }
      b.writeln('    </trkseg>');
      b.writeln('  </trk>');
      b.writeln('</gpx>');
  ...
  Future<File?> _saveKml(File csvFile) async {
    try {
      ...
      final name =
          "track_${_label.replaceAll(RegExp(r'\\W+'), '_')}_$stamp.kml";
      final f = File("${dir.path}/$name");
      final b = StringBuffer();
      b.writeln('<?xml version="1.0" encoding="UTF-8"?>');
  ...
  Future<void> _showSummarySheet(
      File f, File? png, File? geo, File? gpx, File? kml) async {
  ...
  Widget buildSummaryRouteMap(List<LatLng> track) {
    ...
    return SizedBox(
      ...
      child: FlutterMap(
        ...
        options: MapOptions(
          initialCenter:
              track.isNotEmpty ? track.first : const LatLng(20, 0),
          initialZoom: 14,
...
class _RawSample {
  _RawSample(this.t, this.roll, this.pitch, this.yaw, this.tempC, this.cadence,
      this.strideM, this.dH);
  double t;
...
class SoftmaxClassifier {
  ...
  void init({required int numClasses, required int inDim}) {
    ...
    W = List.generate(
        C, (_) => List.generate(D, (_) => (rnd.nextDouble() - 0.5) * 0.01));
    b = List.filled(C, 0.0);
  }
  ...
  int predict(List<double> x) {
    final z = List<double>.generate(
        C,
        (k) => List<double>.generate(D, (j) => W[k][j] * x[j])
                .reduce((a, b) => a + b) +
            b[k]);
    final p = _softmax(z);
    int arg = 0;
