...
class ShoeBle {
  ...
  // Gait phase: 0 = STANCE, 1 = SWING.  Broadcast from the firmware's
  // phase characteristic (UUID c005) so graphs never need to approximate
  // stance/swing from cadence.
  final _phase$ = StreamController<int>.broadcast();
  Stream<int> get phase$ => _phase$.stream;
  ...
  /// Public getter so UI code never needs to touch private fields.
  String? get connectedId => _deviceId;

  /// Emits [BleStatus] whenever the adapter state changes
  /// (poweredOn, poweredOff, unauthorized, unsupported, …).
  Stream<BleStatus> get adapterStatus$ => _ble.statusStream;
  /// True when a BLE connection is fully established and all notifications active.
  ...
  // Gait phase: uint8 0=STANCE 1=SWING, notified by firmware at 10 Hz.
  final Uuid phaseUuid   = Uuid.parse("0000c005-0000-1000-8000-00805f9b34fb");

  // ---------------- Subscriptions ----------------
  StreamSubscription<ConnectionStateUpdate>? _connSub;
  StreamSubscription<List<int>>? _sRoll, _sPitch, _sYaw, _sTemp, _sSteps, _sCad, _sStride, _sDh, _sPhase;
  ...
  Future<void> _subscribeAll(int myEpoch) async {
    ...
    // Phase characteristic is uint8: 0=STANCE, 1=SWING.
    // Wrap in try/catch — older firmware without this characteristic will
    // throw on subscribe; the stream simply stays silent in that case.
    try {
      _sPhase = await sub(phaseUuid, (b) {
        if (b.isNotEmpty) _safeAdd(_phase$, b[0] & 0xFF);
      });
    } catch (_) {}
  }
  ...
  Future<void> _teardownNotifies({required bool cancelNative}) async {
    ...
    if (cancelNative) {
      ...
      await _cancel(_sStride); await _cancel(_sDh);     await _cancel(_sPhase);
    }
    _sRoll = _sPitch = _sYaw = _sTemp = _sSteps = _sCad = _sStride = _sDh = _sPhase = null;
  }
  ...
  // Firmware sends stepCount as uint32_t — use getUint32, not getInt32,
  // to avoid sign-flip for step counts above 2^31.
  int _i32(List<int> b) {
  ...
  /// Re-enables the background scanner and attempts to restore the last
  /// BLE session if it dropped while the screen was off.
  Future<void> onAppResumed() async {
    if (_isConnected) return;
    _autoReconnect = true;
    _startBackgroundScanner();
    await tryReconnectLast(delay: const Duration(milliseconds: 500));
  }

  /// Called by AppLifecycleState handler — cancels reconnect timers but does
  /// not wipe the persisted device id, so the next foreground launch can
  ...
  /// Tries to reconnect to the last saved device; returns true if a BLE
  /// connection was established and the first roll packet arrived.
  /// ConnectHomePage expects Future<bool>.
  Future<bool> reconnectLastIfAny() async {
    if (_disposed) return false;
    if (_isConnected) return true;

    await _loadLast();
    if (_lastId == null || _lastId!.isEmpty) return false;

    // Fast path — try stored id directly.
    try { await connect(_lastId!); } catch (_) {}

    // Wait up to 5 s for a real data packet as confirmation.
    try {
      await roll$.first.timeout(const Duration(seconds: 5));
      return true;
    } catch (_) {}

    // Fallback — scan and try whatever device we find.
    final hit = await scanOncePreferLast(timeout: const Duration(seconds: 4));
    if (hit == null) return false;
    try { await connect(hit.id); } catch (_) { return false; }
    try {
      await roll$.first.timeout(const Duration(seconds: 5));
      return true;
    } catch (_) {
