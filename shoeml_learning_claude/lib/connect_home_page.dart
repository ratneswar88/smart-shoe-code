...
///   * keep trying reconnect in the background on a loop
...
class ConnectHomePage extends StatefulWidget {
  ...
  final Stream<BleStatus>? bleStatus$;
...
class _ConnectHomePageState extends State<ConnectHomePage> {
  ...
  void _bindBleStatus() {
    if (widget.bleStatus$ == null) return;
    _bleStatusSub = widget.bleStatus$!.listen((status) {
      if (!mounted) return;
  ...
  void _startReconnectLoop() {
    ...
    _retryTimer = Timer.periodic(const Duration(seconds: 8), (_) {
      if (!mounted || _navigated || _isConnected) return;
      _tryReconnectOnce();
    });
  }

  Future<void> _tryReconnectOnce() async {
    if (_isTryingReconnect || _isConnected || _navigated) return;
    ...
    try {
      ...
      setState(() {
        _isTryingReconnect = false;
        _statusText = 'Reconnect failed. Waiting for next retry...';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // BLE adapter warning banner
            if (_bleStatus == BleStatus.poweredOff)
              MaterialBanner(
                backgroundColor: Theme.of(context).colorScheme.errorContainer,
                content: Text(
                  'Bluetooth is off. Turn it on to connect to the shoe.',
                  style: TextStyle(
          ...
          children: [
            ...
            Text(
              _hasSeenWelcome
                  ? 'When the sensor is disconnected, the app stays on this home page and keeps retrying the reconnect in the background. You can still open the dashboard manually at any time.'
                  : 'First launch stays on this welcome page. After that, the app will wait here for sensor reconnect, keep retrying in the background, and you can still open the dashboard manually whenever you want.',
