...
class ShoeBle {
  ...
  Future<void> disconnect() async {
    ...
    // Explicitly notify listeners that we are now disconnected
  ...
  Future<void> reconnectLastIfAny() async {
    try {
      ...
      _resetAnalytics(); // new session → reset analytics
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  LoadedActivityModel? _liveModel;
  String? _liveModelName;
  ...
  void _onNewLiveRoll(double v) {
    // Only buffer when a model is loaded
    if (_liveModel == null) return;
  ...
  void _tryClassifyLive() {
    ...
    final pred = model.predictLabel(feat);

    setState(() {
      _liveActivity = pred.key;
      _liveConf = pred.value;
    });
  }
  ...
  Future<void> _loadActiveModel() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final path = prefs.getString(kActiveModelPathKey);
      ...
      if (path == null || path.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "No active model selected. Go to Training → \"Active model for live classification\" and choose one.",
            ),
          ),
        );
        return;
      ...
      if (!await f.exists()) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "Active model file not found. Pick a model again on the Training tab.",
            ),
          ),
        );
        return;
      ...
      if (decoded is! Map<String, dynamic>) {
        throw Exception("Invalid model JSON format");
      }
      ...
      setState(() {
        _liveModel = m;
        _liveModelName = f.path.split('/').last;
        _liveBuf.clear();
        _tLive = 0.0;
        _liveActivity = null;
        _liveConf = null;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Loaded active model: ${_liveModelName} (${m.labels.length} classes)",
          ),
        ),
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Failed to load active model: $e"),
        ),
      );
    } finally {
      if (mounted) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              model == null
                  ? "Train a model on the Training tab, then use \"Active model for live classification\" to select which JSON to use. Finally, tap \"Load active model\" here to start live walking / running / stairs prediction."
                  : "Using model with ${model.labels.length} classes. Prediction is updated from a ~3-second IMU window.",
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 6),
            Text(
              "Active model: ${_liveModelName ?? 'none (set on Training tab)'}",
              style: Theme.of(context)
                  .textTheme
                  .bodySmall
...
class _TrainingPageState extends State<TrainingPage> {
  ...
  Future<void> _trainAndSave() async {
    ...
    // Label per window = majority labelIdx of samples in that window
    final labels = <int>[];
    for (final w in wins) {
    ...
    final nTrain = (0.8 * n).floor().clamp(1, n - 1);
    final Xtr = feats.sublist(0, nTrain);
    final Ytr = labels.sublist(0, nTrain);
    final Xte = feats.sublist(nTrain);
    final Yte = labels.sublist(nTrain);
    ...
    // refresh model list / active info
  ...
  Future<void> _loadModelsAndActive() async {
    ...
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final active = prefs.getString(kActiveModelPathKey);

      if (mounted) {
        setState(() {
          _modelFiles = models;
          _activeModelPath = active;
        });
      }
    } catch (_) {
      if (mounted) {
  ...
  Future<void> _setActiveModel(File f) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(kActiveModelPathKey, f.path);
      if (!mounted) return;
      setState(() {
        _activeModelPath = f.path;
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Active model set: ${f.path.split('/').last}",
          ),
        ),
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Failed to set active model: $e"),
        ),
      );
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              "Pick which trained JSON model should be used when you tap \"Load active model\" on the Dashboard.",
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 12),
            if (_loadingModels)
              ...
              Column(
                children: _modelFiles.map((f) {
                  final name = f.path.split('/').last;
                  final isActive = (f.path == _activeModelPath);
                  return ListTile(
                    dense: true,
                    ...
                    trailing: isActive
                        ...
                        : null,
                    onTap: () => _setActiveModel(f),
                  );
                }).toList(),
              ),
            const SizedBox(height: 8),
            Text(
              "Current active: ${_activeModelPath != null ? _activeModelPath!.split('/').last : 'none'}",
              style: Theme.of(context).textTheme.bodySmall,
            ),
          ],
        ),
  ...
  Future<void> _startLocation() async {
    ...
    _posSub = Geolocator.getPositionStream(
      locationSettings: const LocationSettings(
        ...
        distanceFilter: 3, // ~3 m before new point
      ),
    ).listen((pos) {
  ...
  Future<void> _showShareReportsPicker() async {
    ...
    if (files.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("No PDF reports found yet. Generate one from a session first."),
        ),
      );
      return;
    }
          ...
          child: Column(
            ...
            children: [
              const ListTile(
                ...
                subtitle: Text(
                  "Tap to view / download • use the compare icon to analyze vs other sessions",
                ),
              ),
              const Divider(height: 1),
              Flexible(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      title: Text(name),
                      onTap: () async {
                        // NEW: open actions (Open / Download / Share)
                        Navigator.of(ctx).pop();
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () async {
                          ...
                          if (csv == null || !csv.existsSync()) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text("No matching CSV found for $name"),
                              ),
                            );
                            return;
                          }
...
class LoadedActivityModel {
  ...
  /// Returns (bestLabel, probability)
  MapEntry<String, double> predictLabel(List<double> x) {
    final z = _z(x);
    if (z.isEmpty) return const MapEntry<String, double>('-', 0.0);
    ...
    final label = (bestIdx >= 0 && bestIdx < labels.length)
        ? labels[bestIdx]
        : "class_$bestIdx";
    return MapEntry(label, bestP);
  }
}
